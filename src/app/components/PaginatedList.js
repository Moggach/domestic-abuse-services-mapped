  ...
  return (
    <div className="flex justify-center items-center mt-4">
      <button
        ...
        className="px-4 py-2 mr-2 bg-gray-200 disabled:bg-gray-400"
      >
        Previous
      </button>
      ...
      <button
        ...
        className="px-4 py-2 ml-2 bg-gray-200 disabled:bg-gray-400"
      >
        Next
      </button>
    </div>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  {Array.isArray(properties.serviceType)
                    ? properties.serviceType.map((type, i) => (
                      <div key={i} className={`badge ${getColorForBadge(type)} p-5 text-white text-sm`}>
                        {type}
                      </div>
                    ))
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    {Array.isArray(properties.serviceSpecialism)
                      ? properties.serviceSpecialism.map((spec, i) => (
                        <div key={i} className={`badge ${getColorForBadge(spec)} p-5 text-sm`}>
                          {spec}
                        </div>
                      ))
